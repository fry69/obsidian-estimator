...
// ): Promise<ArrayBuffer> {
//   const normalizedData: BufferSource =
//     typeof data === "string" ? enc.encode(data) : data;
//   const encryptedData = await encryptData(normalizedData, password, iterations);

//   try {
//     await namespace.put(key, encryptedData, options);
//     return encryptedData;
//   } catch (error) {
//     throw new PutError(`Error putting value to kv: ${toErrorMessage(error)}`);
//   }
// }
 ...
 * */
export async function getDecryptedKV(
  namespace: KVNamespace,
  key: string,
  password: string,
): Promise<ArrayBuffer> {
  ...
  const decryptedData = await decryptData(kvEncryptedData, password);
...
export async function encryptData(
  ...
  try {
    ...
    const iterationsArr = new Uint8Array(enc.encode(iterations.toString()));
    ...
    throw new EncryptionError(
      `Error encrypting value: ${toErrorMessage(error)}`,
    );
  }
}
...
async function decryptData(
  ...
  try {
    ...
    throw new DecryptionError(
      `Error decrypting value: ${toErrorMessage(error)}`,
    );
  }
}
